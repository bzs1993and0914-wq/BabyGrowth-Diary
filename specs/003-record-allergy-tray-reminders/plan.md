...
在现有 **Electron + Vue 3 + FastAPI + SQLAlchemy/SQLite** 架构上交付阶段三能力：（1）创建/编辑记录时日期不得超过本地「今天」，前后端双重校验；（2）每日记录增加「宝宝过敏食物」文本字段，首页入口进入聚合列表，时间轴/记录列表醒目标识；（3）桌面应用在每个自然日本地 17:00 至多一次系统通知式提醒（文案固定含义）；（4）关闭主窗口时隐藏窗口并驻留系统托盘，点击托盘图标恢复主窗口，托盘菜单提供「退出」以结束进程。
